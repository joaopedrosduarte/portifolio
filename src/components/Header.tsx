    ...
    return (
        <header className='bg-darkmode-bg shadow-lg w-full py-6 fixed z-50 flex justify-center align-middle'>
            <nav className="flex justify-between max-w-5xl w-full px-10">
                ...
                {links.map((link) => (  
                    <a className='rg:bg-darkmode-bg text-lg bg-darkmode-bg flex flex-1 justify-center items-center h-max duration-300 rg:hover:text-blue-500 ' href={link.link} key={link.name}>{link.name}</a>
                ))}
